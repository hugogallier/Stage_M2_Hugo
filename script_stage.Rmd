...
lmmodel<-lm(S2~Site,data=dataR3)
ncvTest(lmmodel) #si supérieur à alpha alors homoscedastique

#normalité des valeurs
shapiro_test(lmmodel$residuals)
hist(dataR3$S2)
...
model1111<-gam(S2~s(Day_delta)+Site+saison,data=dataR3)
model11111<-gam(S2~s(Day_delta)+Structure2+saison,data=dataR3)
model111111<-gam(S2~s(Day_delta)+Structure2+Site,data=dataR3)
model1111111<-gam(S2~s(Day_delta)+Structure2+Site+saison,data=dataR3)
...
model22222<-gam(S2~Site/Day_delta+Day_delta+Structure2+Site+saison,data=dataR3)

summary(model222)

AICc(model1,model11,model111,model1111,model11111,model111111,model1111111,model2,model22,model222,model2222,model22222)
...
#matrice présence/absence moyenne par site par espèce
dataR2=dataR %>% ungroup %>%
...
Piélouind<-shannonind/(specnumber(dataR4[,-1]))
...
datatern<-cbind(datadissi[,-c(4:5,7)],Dissimilarité=(1-datadissi[,7])*100,Remplacement=100*datarepl[,7],Richesse=100*datarich[,7])


########## Test package gg plot ########## 

 #http://www.ggtern.com/d/2.2.0/
ggtern(data=datatern2, aes(x=Richesse,y=Dissimilarité,z=Remplacement)) +
...
ggtern(data=datatern2, aes(x=Richesse,y=Dissimilarité,z=Remplacement)) +
  ...
  stat_density_tern( #ajout de la densité  
    geom="polygon",aes(fill=..level..),bins=5,color='grey')+
  labs(title="")+
  # theme_arrowlarge()+
  theme_noarrows()+
theme_custom(  #https://rdrr.io/cran/ggtern/man/ggtern_themes.html
