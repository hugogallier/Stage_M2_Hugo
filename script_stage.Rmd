...
dataR=dataR1 %>% filter(!spp %in% c("NI","Unassigned","ponte","Ponte",'None','NA')) %>%
  ...
  select (spp,abb,Site,Micro,Date_courte,Structure2,Mcov,Day_delta)
...
#avec Mcov
dataR2=dataR %>% ungroup %>% 
  select(spp,Site,Structure2,Micro,Day_delta,saison,Date_courte,Mcov) %>% 
  distinct()%>% 
  mutate(Years=substr(Date_courte, start = 1, stop = 4)) %>%
  group_by(Site,Structure2,Day_delta,saison,Years,Mcov) %>% 
  # summarize(S=n()) %>%
  na.omit()%>% 
  droplevels()
...
dataR3=dataR2 %>% 
  group_by(Site,Structure2,Day_delta,saison) %>% 
  summarize(Mcov2 = sum(Mcov,.by=Structure2)) 
...
# dataR3$Day_delta<-as.factor(dataR3$Day_delta)
...
model1<-gam(Mcov~s(Day_delta)+Site,data=dataR3)
...
plot(model11111111,shift=mean(dataR3$S2),residuals=T,cex=0.7,pch=16,col="blue",shade=T,shade.col='grey')
ggplot(data = dataR3, mapping = aes(x = Day_delta, y = S2)) +
  geom_point(size = 0.7, alpha = 0.7) +
  geom_smooth(method="gam", formula= y~s(x))+
  theme_bw()

plot_smooths(model=model11111111, series=Day_delta,comparison=Site)+theme_bw() #permet de mettre les gam selon les sites
...
model11111<-gam(S2~s(Day_delta)+Site,data=dataR3)
model111111<-gam(S2~s(Day_delta)+Structure2,data=dataR3)
model1111111<-gam(S2~s(Day_delta)+Structure2+Site,data=dataR3)
model11111111<-gam(S2~s(Day_delta)+s(Structure2,bs="re")+s(Site,bs="re"),data=dataR3)
model111111111<-gam(Mcov2~s(Day_delta)+s(Structure2,bs='re')+s(Site,bs="re")+Site,data=dataR3)
...
model222222222222<-gam(S2~Site/Day_delta+Day_delta+Structure2+Site+Years,data=dataR3)
...
fm1<-gam(S2~s(Day_delta)+s(Structure2,bs="re")+s(Site,bs="re"),data=dataR3)
ms1<-dredge(fm1)
par(mar = c(3,5,6,4))
...
Comp_mat=data_comp2 %>% select(-c(Site,Micro,Structure2,Date_courte,saison,Years)) %>% as.matrix() #sans meta données
Meta_data=data_comp2 %>% select(Site,Micro,Structure2,Date_courte,saison,Years) #meta données
...
res.pca<-PCA(PCOA_df[,c(1:2)])
res.pca<-PCA(x,y)
res.pca<-PCA(Comp_mat,ncp=6)
res_va<-dimdesc(res.pca,axes=c(1,2))#permet d'avoir la corrélation
...
Piélouind<-shannonind/45 #indice de shannon divisé par le nombre max de taxons sur un meme site
...
databetadiv2<-beta.div.comp(dataR2[,-c(1:4,65)],coef="S") #calcul de la nestedness, diversité et turnover
databetadiv2$rich
...
Vérification de la section betadiv. Résolution du problème somme != 0.
```{r}
#### Test package adespatial
data_comp=data_PQ_light %>% ungroup %>% 
  select(spp,Site,Micro,Date_courte,Structure2,Mcov) %>% 
  group_by(spp,Site,Micro,Date_courte,Structure2,Mcov) %>% 
  distinct() %>% drop_na() %>% 
    spread(spp,Mcov) %>% ungroup() %>% 
...
# On construit la matrice de communauté en retirant les metadata des quadrats
# Mais aussi  les "None" + "Ni" + "Ponte" + "ponte"
data_comp_mat=data_comp %>% dplyr::select(-c(Site,Micro,Date_courte,Structure2,
                                          Unassigned,Name, NI, None,
                                          Ponte,ponte))
...
mat.D[upper.tri(mat.D)]<-NA
diag(mat.D)<-NA
rownames(mat.D)<-data_comp$Name #change le nom des lignes avec les metadonnées
colnames(mat.D)<-data_comp$Name #change le nom des colonnes avec les metadonnées
 
mat.repl=as.matrix(databetadiv2$repl)
mat.repl[upper.tri(mat.repl)]<-NA
diag(mat.repl)<-NA
rownames(mat.repl)<-data_comp$Name #change le nom des lignes avec les metadonnées
colnames(mat.repl)<-data_comp$Name #change le nom des colonnes avec les metadonnées
 
mat.rich=as.matrix(databetadiv2$rich)
mat.rich[upper.tri(mat.rich)]<-NA
diag(mat.rich)<-NA
rownames(mat.rich)<-data_comp$Name #change le nom des lignes avec les metadonnées
colnames(mat.rich)<-data_comp$Name #change le nom des colonnes avec les metadonnées
...
# Il y a un problème ici. Tu retires la colonne structure 2 (-5) et donc après tu ne sais plus quelles sont les structures qui csont comparées
...
# Test avec la variabilité intra des Inner_slots (Structure1==Structure2)
...
# j'ai refait cette ligne pour éviter de se tromper avec les numéros de colonnes
# datamean<-as.data.frame(t(colMeans(datatern2[,c(6:8)])))
datamean<-datatern2 %>%  group_by(Comp_site) %>%  
...
ggtern(data=datatern2, aes(x=Richesse,y=Similarité,z=Remplacement)) +
  geom_point(colour='black',alpha=0.3,size=1,
...
dataR2=data_PQ_light %>% ungroup %>%
  ...
  filter(!Structure2 %in% c('Holes','Upper_slots','Dome')) %>% 
  filter(!Date_courte == '2021-01') %>% 
  select(Mcov,Site,Date_courte,Structure2) %>% 
  group_by(Mcov,Site,Date_courte,Structure2) %>% 
  na.omit()

dataR3=dataR2 %>% as.data.frame() %>% 
  summarize(Mcov2=mean(Mcov),.by = c(Date_courte,Site)) %>%
  na.omit()

ggplot(dataR2, aes(Structure2,Mcov, fill=Site))+
  xlab("Structure")+
...
ggplot(dataR3, aes(Date_courte,Mcov2, col=Site))+
  xlab("Date")+
