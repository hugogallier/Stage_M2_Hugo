...
ggplot(dataR2 %>%  filter(!Structure2 == "Holes") %>% filter(!Date_courte=="2021-01"), aes(Date_courte,S, col=Site))+
  xlab("Date")+
  ylab("Richesse taxonomique")+
  geom_point()+
...
ggplot(dataR2 %>%  filter(!Structure2 %in% c("Holes","Upper_slots","Dome")) %>% filter(!Date_courte=="2021-01"), aes(Date_courte,S, col=Structure2))+
  xlab("Date")+
  ylab("Richesse taxonomique")+
  geom_point()+
  geom_line( aes(group=Site))+
...
ggradar(resultat3[,-2],grid.min = 1,grid.max =15,values.radar = c(0,7, 15),group.colours = lcol,legend.title="Site")
...
ggradar(resultat3[,-2],grid.min = 1,grid.max =10,grid.mid=5,values.radar = c(0,5,10),group.colours = lcol,legend.title="Site",gridline.max.linetype = 1,gridline.max.colour = "black",background.circle.colour = "lightblue")


##### Upper_slots #####
resultat4<- spread((dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned")) %>%#enlève les NI
  filter(!Structure2 %in% c("Dome","Holes")) %>%
  # filter(!Date_courte == ("2021-01"))%>%
  filter(Micro %in% c("CGF1","CGF2","CGF3","CGF4"))%>%
  select(spp,Site,Date_courte,Micro) %>% 
  distinct() %>% 
  group_by(Site,Date_courte,Micro) %>% #filtre selon le site, date et structure
  summarize(S=n())),key=Micro,value=S) #créér une nouvelle colonne permettant de faire un summary des valeurs filtrées
...
ggradar(resultat4[,-2],grid.min = 1,grid.max =13,grid.mid=5,values.radar = c(0,5,13),group.colours = lcol,legend.title="Site",gridline.max.linetype = 1,gridline.max.colour = "black",background.circle.colour = "lightblue")


##### Pieds #####
resultat5<- spread((dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned")) %>%#enlève les NI
  filter(!Structure2 %in% c("Dome","Holes")) %>%
  # filter(!Date_courte == ("2021-01"))%>%
  filter(Micro %in% c("P1","P2","P3"))%>%
  select(spp,Site,Date_courte,Micro) %>% 
  distinct() %>% 
  group_by(Site,Date_courte,Micro) %>% #filtre selon le site, date et structure
  summarize(S=n())),key=Micro,value=S) #créér une nouvelle colonne permettant de faire un summary des valeurs filtrées
resultat5[is.na(resultat5)]<-0

ggradar(resultat5[,-2],grid.min = 1,grid.max =14,grid.mid=5,values.radar = c(0,5,14),group.colours = lcol,legend.title="Site",gridline.max.linetype = 1,gridline.max.colour = "black",background.circle.colour = "lightblue")


##### Dome #####
resultat6<- na.omit(spread((dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned")) %>%#enlève les NI
  filter(!Structure2 %in% c("Upper_slots","Holes")) %>%
  filter(!Date_courte == ("2021-01"))%>%
  filter(Micro %in% c("CHB1","CHB2","CHB3","CHB4","CHB5","CHB6","CHB7","CHB8"))%>%
  select(spp,Site,Date_courte,Micro) %>% 
  distinct() %>% 
  group_by(Site,Date_courte,Micro) %>% #filtre selon le site, date et structure
  summarize(S=n())),key=Micro,value=S)) #créér une nouvelle colonne permettant de faire un summary des valeurs filtrées
resultat6[is.na(resultat6)]<-0

ggradar(resultat6[,-2],grid.min = 1,grid.max =9,values.radar = c(0,5,9),group.colours = lcol,legend.title="Site",gridline.max.linetype = 1,gridline.max.colour = "black",background.circle.colour = "lightblue")


##### Inner_slots #####
resultat7<- na.omit(spread((dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned")) %>%#enlève les NI
  filter(!Date_courte == ("2021-01"))%>%
  filter(Micro %in% c("DGF1","DGF2","DGF3","DGF4"))%>%
  select(spp,Site,Date_courte,Micro) %>% 
  distinct() %>% 
  group_by(Site,Date_courte,Micro) %>% #filtre selon le site, date et structure
  summarize(S=n())),key=Micro,value=S)) #créér une nouvelle colonne permettant de faire un summary des valeurs filtrées
resultat7[is.na(resultat7)]<-0

ggradar(resultat7[,-2],grid.min = 1,grid.max =8,values.radar = c(0,3,7),group.colours = lcol,legend.title="Site",gridline.max.linetype = 1,gridline.max.colour = "black",background.circle.colour = "lightblue")


##### Inner_slots #####
resultat8<- na.omit(spread((dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned")) %>%#enlève les NI
  filter(!Date_courte == ("2021-01"))%>%
  filter(Micro %in% c("DPF1","DPF2","DPF3","DPF4"))%>%
  select(spp,Site,Date_courte,Micro) %>% 
  distinct() %>% 
  group_by(Site,Date_courte,Micro) %>% #filtre selon le site, date et structure
  summarize(S=n())),key=Micro,value=S)) #créér une nouvelle colonne permettant de faire un summary des valeurs filtrées
resultat8[is.na(resultat8)]<-0

ggradar(resultat8[,-2],grid.min = 1,grid.max =6,values.radar = c(0,3,6),group.colours = lcol,legend.title="Site",gridline.max.linetype = 1,gridline.max.colour = "black",background.circle.colour = "lightblue")
