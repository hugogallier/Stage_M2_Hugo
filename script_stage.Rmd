...
dataR2=dataR %>% ungroup %>% 
  filter(!Date_courte=="2021-01") %>%
  select(spp,Site,Date_courte,Structure2,nummois) %>% 
  distinct() %>% 
  group_by(Site,nummois,Structure2,Date_courte) %>% #filtre selon le site, date et structure
  summarize(S=n()) 
 
glm1<-glm(S~Site+Structure2+nummois+Date_courte,data=dataR2)
plot(glm1)
summary(glm1)
```
