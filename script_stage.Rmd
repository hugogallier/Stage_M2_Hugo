...
plot(out.raw,xlim=c(0,150),ylim=c(0,80))
...
datamean<-datatern2 %>%  group_by(Compasite,Compasaison) %>% 
...
ggtern(data=datatern2, aes(x=Richesse,y=Similarité,z=Remplacement,group=Compasaison)) +
  ...
  geom_point(colour="grey",position=position_jitter_tern(x=0.1, y=0.1, z=0.1),alpha=.3)+
        geom_point(data=datamean,aes(colour=Compasaison))+
  ...
  theme_showarrows()+ #montre les fléches
    geom_crosshair_tern(data=datamean,aes(colour=Compasaison))+
...
ggsave("Compasite_saison.png",dpi=350,height = 10,width=12)
