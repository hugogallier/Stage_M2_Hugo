...
dataR21=dataR %>% ungroup %>%   
  select(spp,Site,Date_courte,Day_delta) %>% 
  filter(!Date_courte=="2021-01") %>%
  filter(Site %in% c('Bi')) %>% 
  # filter(Structure2=='Disc') %>% 
  distinct() %>% 
  group_by(Site,Day_delta) %>% #filtre selon le site, date et structure
  summarize(S=n()) 
...
Comp_mat=data_comp2 %>% select(-c(Site,Micro,Structure2,Date_courte,nouvelle,saison)) %>% as.matrix() 
Meta_data=data_comp2 %>% select(Site,Micro,Structure2,Date_courte,nouvelle,saison) #créer jeu de données avec les méta données
...
ggplot(data=PCOA_df,aes(x,y, col=Structure2))+ # <- Ici on explore avec col les variable qui explique le + l'explosion du nuage de points
  # geom_point(aes(col=Site), size=1, alpha=.6)+
  geom_point()+
  labs(x="Axe 1 (temporalité)",y="Axe 2")+
  geom_hline(yintercept=0)+
...
ggplot(datanumber, aes(Date,Disques, fill=Site))+ # Avec "fill" ou "col" du peut ajout une séparation de tes données par facteur (ici le site)
  xlab("Structure")+
  ylab("Richesse taxonomique")+
  # geom_jitter(alpha=.3)+  # On ajoute les points (pas nécessaire mais utile pour savoir sur quoi se base les boxplots)
  geom_line()+
...
dataR2<- spread((dataR2=dataR %>% 
  select(spp,Site,Date_courte,Structure2) %>% 
  # filter(Date_courte %in% c("2022-04","2022-07","2022-11","2022-01"))%>%
  filter(Structure2 == ("Disc")) %>%
  # filter(Site == "VB") %>%
  distinct() %>% 
  group_by(spp,Site) %>% 
...
library(bipartite)
seuil<-1
presence_absence <- dataR2 >= seuil
nes<-nestedness(presence_absence)

nested(presence_absence,method="NODF") #valeur élevée = forte Nestedness
