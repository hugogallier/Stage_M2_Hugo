...
test_anova<-aov(S2~Structure2,data=dataR3)#anova en prenant la date ainsi que les structures comme facteur
...
dataR2=dataR %>% ungroup %>%
  # filter(saison == 'été') %>% 
  # filter(Site %in% c("Bi","Bu")) %>% 
  # filter(Date_courte %in% c("2021-04")) %>% 
  select(spp,Site,Date_courte,Structure2,Mcov) %>% 
  distinct()%>% 
  group_by(spp,Site,Date_courte,Structure2,Mcov) %>% 
  na.omit()
...
databetadiv<-beta.div.comp(dataR2[,-c(1:4)],coef="S")
databetadiv$rich
mean(databetadiv$rich)


## Utilisation de la matrice créée
# Extraction des matrices
mat.D=as.matrix(Beta_div$D)
mat.D[upper.tri(mat.D)]<-NA
diag(mat.D)<-NA
 
mat.repl=as.matrix(Beta_div$repl)
mat.repl[upper.tri(mat.repl)]<-NA
diag(mat.repl)<-NA
 
mat.rich=as.matrix(Beta_div$rich)
mat.rich[upper.tri(mat.rich)]<-NA
diag(mat.rich)<-NA
 
 
# Transformation matrices en df
...
nes$statistic #nestedness
turn$total#turnover

datatern<-cbind(Simi=50,NES=nes$statistic,Turn=100*mean(turn$total))
