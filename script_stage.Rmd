...
# ggplot(dataR3, aes(Structure2,S2,fill=Structure2))+ #Structure
  ...
  geom_bar(stat="identity",alpha=.7)+
      scale_fill_manual(values = c("#ff6633", "#99CCFF", "#339966","gold1"))+
...
lm1<-lm(S2~Date_courte,data=dataR3) #pour voir quelle est la tendance temporelle
summary(lm1)


adonis2(dataR2$S~Structure2*Date_courte,data=dataR2,permutations=999) #anova pas utilisable car non normale et homoscédastique
adonis2(dataR2$S~Site*Date_courte,data=dataR2,permutations=999)

groupe<-dataR2$Structure2
groupe<-dataR2$Site
adonis3<-permanova_pairwise(dataR2[,-c(1:4)],groupe,permutations=999,method='bray',padj="bonferroni")


plot(aov1$residuals,ylab="Résidus")
leveneTest(dataR3$S2~dataR3$Site)
leveneTest(dataR3$S2~dataR3$Structure)

# qqPlot(sDlengthtot$value)
qqPlot(aov1$residuals)
shapiro.test(x = aov1$residuals )
...
model111111<-gam(S2~s(Day_delta)+s(Structure2,bs="re")+s(Site,bs="re")+s(saison,bs="re")+s(Structure2,by=Site,bs='re')+Structure*Day_delta,data=dataR3) 
model111111<-gam(S2~s(Day_delta)+s(Structure2,bs="re")+s(Site,bs="re")+s(saison,bs="re")+s(Structure2,by=Site,bs='re')+Site*Day_delta,data=dataR3,method="REML") 
...
plot_smooths(model=model111111, series=Day_delta,comparison=Structure)+theme_bw()+
  theme(axis.text.x = element_text(size=16,color="black"))+
  theme(axis.text.y = element_text(size=16,color="black"))+
  theme(axis.title.x = element_text(size = 14))+
 theme(axis.title.y = element_text(size = 14))+
    scale_fill_manual(values = c("#ff6633", "#99CCFF", "#339966","gold1"))+
  ylab("Richesse taxonomique moyenne")+xlab("Jours depuis immersion")
ggsave("gam_influencestructure.png",width=9,height=8,dpi=350)

#test comparaison tous les paramètres
library(MuMIn)
options(na.action = "na.fail")
fm1<-gam(S2~s(Day_delta)+s(Structure2,bs="re")+s(Site,bs="re")+s(saison,bs="re")+s(Structure2,by=Site,bs='re')+Site*Day_delta,data=dataR3)
ms1<-dredge(fm1)
...
simulationOutput <- simulateResiduals(fittedModel = model111111)
...
dataR$temp[datamean$Site1 == "recif1" | datamean$Site1 == "recif2" |datamean$Site1 == "recif3"] <- "48.791513"
...
group<-Meta_data$Site
group<-Meta_data$Date_courte
group<-Meta_data$Structure2

adonis2(distdataR~group,permutations=999)
...
pcoa_tot_SS<- cmdscale(d = as.matrix(distdataR), k=6, eig = T) 
...
PCOA_df=PCOA_df %>% mutate(Structure = fct_recode(Structure, Disques = "Disc", Piliers = "Pillars", Colonnes = "Columns", Failles="Inner_slots"))
...
ggplot(PCOA_df, aes(x = x, y = y,colour =Site )) +
  geom_point(alpha=.2,aes(colour=Site)) +
      scale_color_manual(values = c("#ff6633", "#99CCFF", "#339966","gold1"))+
...
ggplot(PCOA_df, aes(x = x, y = y,colour =Structure )) +
  ...
  theme(axis.title.y = element_text(size=16,colour="black",vjust=2))+
  geom_label(
    label="Disques", x=barycenters$x[which(barycenters$Structure=="Disques")], y=barycenters$y[which(barycenters$Structure=="Disques")],
    label.padding = unit(0.25, "lines"), # Rectangle size around label
    label.size = 0.35,color = "#fb746b",fill="white")+
  geom_label(
    label="Colonnes", x=barycenters$x[which(barycenters$Structure=="Colonnes")], y=barycenters$y[which(barycenters$Structure=="Colonnes")],
    label.padding = unit(0.25, "lines"), 
    label.size = 0.35,color = "#7cab04",fill="white")+
  geom_label(
    label="Piliers", x=barycenters$x[which(barycenters$Structure=="Piliers")], y=barycenters$y[which(barycenters$Structure=="Piliers")],
    label.padding = unit(0.25, "lines"), 
    label.size = 0.35,color = "#00bfc4",fill="white")+
  geom_label(
    label="Failles", x=barycenters$x[which(barycenters$Structure=="Failles")], y=barycenters$y[which(barycenters$Structure=="Failles")],
    label.padding = unit(0.25, "lines"), 
    label.size = 0.35,color = "#c77cff",fill="white",position=)+
...
ggplot(PCOA_df, aes(x = x, y = y,colour =Site )) +
  ...
  # scale_color_manual(values = c("gray70", "gray58", "gray33","gray0"))+
     # Ajout lignes entre les barycentres et les points
  geom_segment(data = barycenters[which(barycenters$Site=="Bi"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi"),],aes(x=x[6],y=y[6],xend=x[7],yend=y[7]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB"),],aes(x=x[6],y=y[6],xend=x[7],yend=y[7]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB"),],aes(x=x[7],y=y[7],xend=x[8],yend=y[8]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
...
ggplot(PCOA_df, aes(x = x, y = y,colour = Site)) +
  ...
  # scale_color_manual(values = c("gray70", "gray58", "gray33","gray0"))+
    # Ajout lignes entre les barycentres et les points
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Disc"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Disc"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Disc"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Disc"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Disc"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Disc"),],aes(x=x[6],y=y[6],xend=x[7],yend=y[7]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Disc"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Disc"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Disc"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Disc"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Disc"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Disc"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Disc"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Disc"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Disc"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Disc"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Disc"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Disc"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="gold1")+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Disc"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Disc"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Disc"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Disc"),],aes(x=x[6],y=y[6],xend=x[7],yend=y[7]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Disc"),],aes(x=x[7],y=y[7],xend=x[8],yend=y[8]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  labs(color="Sites et dates")+ labs(x="Axe 1 (22,1%)",y="Axe 2 (12,2%)")+ theme_bw() +
  theme(axis.text.x = element_text(size=16,colour="black"))+
   theme(axis.text.y = element_text(size=16,colour="black"))+
   theme(axis.title.x = element_text(size=16,colour="black",vjust=0))+
...
ggplot(PCOA_df, aes(x = x, y = y,colour = Site)) +
 geom_point(alpha=.6,color="grey") +
  ...
  # scale_color_manual(values = c("gray70", "gray58", "gray33","gray0"))+
    # Ajout lignes entre les barycentres et les points
   geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Columns"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Columns"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Columns"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Columns"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Columns"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Columns"),],aes(x=x[6],y=y[6],xend=x[7],yend=y[7]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Columns"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Columns"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Columns"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Columns"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Columns"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Columns"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Columns"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Columns"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Columns"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Columns"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Columns"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Columns"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="gold1")+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Columns"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Columns"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Columns"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Columns"),],aes(x=x[6],y=y[6],xend=x[7],yend=y[7]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Columns"),],aes(x=x[7],y=y[7],xend=x[8],yend=y[8]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  labs(color="Sites et dates")+ labs(x="Axe 1 (22,1%)",y="Axe 2 (12,2%)")+ theme_bw() +
  theme(axis.text.x = element_text(size=16,colour="black"))+
   theme(axis.text.y = element_text(size=16,colour="black"))+
   theme(axis.title.x = element_text(size=16,colour="black",vjust=0))+
...
ggplot(PCOA_df, aes(x = x, y = y,colour = Site)) +
  ...
  # scale_color_manual(values = c("gray70", "gray58", "gray33","gray0"))+
    # Ajout lignes entre les barycentres et les points
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Pillars"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Pillars"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Pillars"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Pillars"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Pillars"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Pillars"),],aes(x=x[6],y=y[6],xend=x[7],yend=y[7]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Pillars"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Pillars"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Pillars"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Pillars"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Pillars"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Pillars"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Pillars"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Pillars"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Pillars"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Pillars"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Pillars"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Pillars"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="gold1")+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Pillars"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Pillars"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Pillars"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Pillars"),],aes(x=x[6],y=y[6],xend=x[7],yend=y[7]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Pillars"),],aes(x=x[7],y=y[7],xend=x[8],yend=y[8]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  labs(color="Sites et dates")+ labs(x="Axe 1 (22,1%)",y="Axe 2 (12,2%)")+ theme_bw()+  
 theme(axis.text.x = element_text(size=16,colour="black"))+
   theme(axis.text.y = element_text(size=16,colour="black"))+
   theme(axis.title.x = element_text(size=16,colour="black",vjust=0))+
...
ggplot(PCOA_df, aes(x = x, y = y,colour = Site)) +
 geom_point(alpha=.6,color="grey") +
  ...
  # scale_color_manual(values = c("gray70", "gray58", "gray33","gray0"))+
    # Ajout lignes entre les barycentres et les points
   geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Inner_slots"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Inner_slots"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Inner_slots"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Inner_slots"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Inner_slots"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bi" & barycenters$Structure=="Inner_slots"),],aes(x=x[6],y=y[6],xend=x[7],yend=y[7]),color="#ff6633",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Inner_slots"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Inner_slots"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Inner_slots"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Inner_slots"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Bu" & barycenters$Structure=="Inner_slots"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#99CCFF",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Inner_slots"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Inner_slots"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Inner_slots"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Inner_slots"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="Fe" & barycenters$Structure=="Inner_slots"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="#339966",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Inner_slots"),],aes(x=x[1],y=y[1],xend=x[2],yend=y[2]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Inner_slots"),],aes(x=x[2],y=y[2],xend=x[3],yend=y[3]),color="gold1")+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Inner_slots"),],aes(x=x[3],y=y[3],xend=x[4],yend=y[4]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Inner_slots"),],aes(x=x[4],y=y[4],xend=x[5],yend=y[5]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Inner_slots"),],aes(x=x[5],y=y[5],xend=x[6],yend=y[6]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Inner_slots"),],aes(x=x[6],y=y[6],xend=x[7],yend=y[7]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  geom_segment(data = barycenters[which(barycenters$Site=="VB" & barycenters$Structure=="Inner_slots"),],aes(x=x[7],y=y[7],xend=x[8],yend=y[8]),color="gold1",arrow=arrow(type="closed",length=unit(0.07,"inches"),ends="last"))+
  labs(color="Sites et dates")+ labs(x="Axe 1 (22,1%)",y="Axe 2 (12,2%)")+ theme_bw()+
   theme(axis.text.x = element_text(size=16,colour="black"))+
   theme(axis.text.y = element_text(size=16,colour="black"))+
   theme(axis.title.x = element_text(size=16,colour="black",vjust=0))+
...
summaryPCOA = summaryPCOA %>% mutate(num = seq(1:nrow(summaryPCOA)))
plot(summaryPCOA$PCTVAR[1:30],ylim=c(0,30),xlab="Composantes",ylab="Pourcentage d'inertie")
...
aov1<-aov(value~Structure*Numinv+Site*Numinv,data=sDlengthtot)
summary(aov1)
...
plot(aov1$residuals,ylab="Résidus")
leveneTest(sDlengthtot$value~sDlengthtot$Structure_site)
...
# sDlength1<-sDlength[,c(1,7,8)]
# sDlength2<-sDlength[,c(2,7,8)]
# sDlength3<-sDlength[,c(3,7,8)]
# sDlength4<-sDlength[,c(4,7,8)]
# sDlength5<-sDlength[,c(5,7,8)]
# colnames(sDlength1)[1]<-"value"
# colnames(sDlength2)[1]<-"value"
# colnames(sDlength3)[1]<-"value"
# colnames(sDlength4)[1]<-"value"
# colnames(sDlength5)[1]<-"value"
# 
# sDlength6<-rbind(sDlength1,sDlength2,sDlength3,sDlength4,sDlength5)
# sDlength6$Date<-seq("1","2","3","4",5)
...
aov1<-aov(value~Structure*Site,data=sDlengthtot)
summary(aov1)
...
#test significativité
aov1<-aov(value~Structure*Numinv+Site*Numinv,data=sDAngletot)
summary(aov1)
...
#test significativité
aov1<-aov(value~Structure*Site,data=sDAngletot)
summary(aov1)
...
data_comp1=data_PQ_light %>% ungroup %>% 
  ...
  mutate(date_st=paste0(Date_courte,'_',Code_struct)) %>%  
   ...
   # filter(Date_courte %in% c("2021-01","2021-04","2021-07","2021-10","2021-11")) %>% 
...
comp_list_clean=lapply(comp_list, fun_clean)
names(comp_list_clean)<-c('Bi','Bu','Fe','VB')
names(comp_list_clean)<-c('Bi','Bu','VB')
...
plot(out.raw,xlim=c(0,150),ylim=c(0,80),col=c("#ff6633", "#99CCFF", "#339966","gold1"))
plot(out.raw,xlim=c(0,150),ylim=c(0,80),col=c("#ff6633", "#99CCFF","gold1")) 
plot(out.raw,xlim=c(0,500),ylim=c(0,80),col=c("#ff6633", "#99CCFF", "#339966","gold1"))


# Ici tu as les données concernant l'estimation de la rich. totale
Rar_disc_df=as.data.frame(out.raw$AsyEst)




#####  Détermination de la richesse en fonction des différents indicateurs
dataR$saison<-dataR$Date_courte
dataR$saison[dataR$saison == "2021-04" | dataR$saison == "2022-04"] <- "printemps"
dataR$saison[dataR$saison == "2021-07" | dataR$saison == "2022-07"] <- "été"
dataR$saison[dataR$saison == "2021-10" | dataR$saison == "2021-11"| dataR$saison == "2022-11"] <- "automne"
...
#permet d'avoir une matrice en binaire
colonnes_a_modifier<-colnames(data_comp2[c(8:ncol(data_comp2))])
data_comp2[colonnes_a_modifier] <- lapply(data_comp2[colonnes_a_modifier], function(colonne) {
  colonne[colonne > 0] <- 1
  return(colonne)
})
...
Comp_mat=data_comp2 %>% select(-c(Site,Micro,Structure2,Date_courte,saison,Years,Day_delta,Compsite_date)) %>% as.matrix()
Meta_data=data_comp2 %>% select(Site,Micro,Structure2,Date_courte,saison,Years,Day_delta,Compsite_date) 


specpool(Comp_mat, Meta_data$Site, smallsample = TRUE)
estimateR(x, ...)
specpool2vect(X, index = c("jack1","jack2", "chao", "boot","Species"))
pa<-poolaccum(Comp_mat, permutations = 100, minsize = 3)
estaccumR(Comp_mat, permutations = 100, parallel = getOption("mc.cores"))
## S3 method for class 'poolaccum'
summary(object, display, alpha = 0.05, ...)
## S3 method for class 'poolaccum'
plot(pa, alpha = 0.05, type = c("l","g"))
...
dataR2=dataR %>% ungroup %>%
  select(spp,Site,Structure2,Micro,Day_delta,Mcov,Date_courte) %>% 
  distinct()%>% 
  group_by(spp,Site,Structure2,Day_delta,Mcov,Date_courte)

dataR4<-spread((dataR3=dataR2 %>% 
  group_by(spp,Site,Day_delta,Date_courte) %>%
  summarize(Mcov2 = mean(Mcov))),key=spp,value=Mcov2) 
dataR4[is.na(dataR4)]<-0

# Calcul indice de Shannon
shannonind<-diversity(dataR4[,-c(1:3)],index="shannon")
shannonind2<-cbind(dataR4[,c(1:3)],shannonind)
colnames(shannonind2)[4]<-"shannon"
...
Piélouind2<-cbind(dataR4[,c(1:3)],Piélouind)
colnames(Piélouind2)[4]<-"Piélou"

Piélouind2$Day_delta<-as.numeric(Piélouind2$Day_delta)
Piélouind2$Piélou<-as.numeric(Piélouind2$Piélou)
...
ggplot(Piélouind2, aes(Day_delta,Piélou, col=Site))+ # Avec "fill" ou "col" du peut ajout une séparation de tes données par facteur (ici le site)
  ...
  # geom_jitter(alpha=.3)+  # On ajoute les points (pas nécessaire mais utile pour savoir sur quoi se base les boxplots)
  # geom_boxplot()+
annotate("rect",xmin=18,xmax=111.9,ymin=0.3,ymax=0.9,alpha=.3,fill="#666666")+
  annotate("rect",xmin=112,xmax=205.9,ymin=0.3,ymax=0.9,alpha=.3,fill="grey")+
  annotate("rect",xmin=206,xmax=298.9,ymin=0.3,ymax=0.9,alpha=.3,fill="#666666")+
  annotate("rect",xmin=299,xmax=388.9,ymin=0.3,ymax = 0.9,alpha=.3,fill="grey")+
  annotate("rect",xmin=389,xmax=477.9,ymin=0.3,ymax = 0.9,alpha=.3,fill="#666666")+
annotate("rect",xmin=478,xmax=570.9,ymin=0.3,ymax = 0.9,alpha=.3,fill="grey")+
  annotate("rect",xmin=571,xmax=663.9,ymin=0.3,ymax = 0.9,alpha=.3,fill="#666666")+
  annotate("rect",xmin=664,xmax=750,ymin=0.3,ymax = 0.9,alpha=.3,fill="grey")+
  theme_bw() +
...
datatern2=datatern2 %>% mutate(Structure1 = fct_recode(Structure1, Piliers = "Pillars", Colonnes = "Columns", Failles="Inner_slots")) %>% mutate(Structure2 = fct_recode(Structure2, Disques = "Disc", Piliers = "Pillars", Colonnes = "Columns"))
...
datatern2=datatern2 %>% 
 mutate(Compasite<-paste(Site1,Site2,sep="/")) %>% 
  ...
  mutate(Compasaison<-paste(Saison1,sep="/")) %>% 
...
# datatern2=datatern2 %>% mutate(Compasite = fct_recode(Compasite, Fe = "Fe", Autres = "Bi", Autres = "Bu", Autres="VB")) 

# datatern2$structint_ext<-rep("Structures externes")
# datatern2$structint_ext[which(datatern2$Compastruct=="Inner_slots")]<-"Structure interne"

datamean<-datatern2 %>%  group_by(Comp_site,Compastruct) %>% 
  dplyr::summarize(Similarité=mean(Similarité),
            Remplacement=mean(Remplacement),
            Richesse=mean(Richesse))

# datamean$Compasaison<-fct_relevel(datamean$Compasaison,c("printemps","été","automne","hiver"))


ggtern(data=datatern2, aes(x=Richesse,y=Similarité,z=Remplacement,group=Comp_site)) +
  ...
  geom_point(colour="grey",position=position_jitter_tern(x=0.1, y=0.1, z=0.1),alpha=.3)+
        geom_point(data=datamean,aes(colour=Comp_site))+
  ...
  theme_showarrows()+ #montre les fléches
    geom_crosshair_tern(data=datamean,aes(colour=Comp_site))+
    theme(strip.text.x = element_text(size = 12))+
  # facet_wrap(.~Comp_struct)+
    facet_wrap(.~Compastruct)+
  labs(title="")
  # scale_color_paletteer_d("dichromat::SteppedSequential_5")
  # labs(color = "Structures comparées")+
ggsave("Compasite_struct.png",dpi=350,height = 10,width=12)
ggsave("Compastruct_site.png",dpi=350,height = 10,width=12)
...
model111111<-gam(Mcov3~s(Day_delta,k=15)+s(Structure2,bs="re")+s(Site,bs="re")+s(saison,bs="re")+s(Structure2,by=Site,bs='re'),data=dataR3,method="REML")
...
data_comp2=data_comp2 %>% mutate(Structure2 = fct_recode(Structure2, Disques = "Disc", Piliers = "Pillars", Colonnes = "Columns", Failles="Inner_slots"))

#faire la moyenne par structure
...
#diviser pour avoir une valeur représentative de l'année
data_comp5$Mcov4<-data_comp5$Mcov4/nlevels(as.factor(data_comp3$Day_delta))
data_comp5$sd<-data_comp5$sd/nlevels(as.factor(data_comp3$Day_delta))

#proportion relative par structure
data_comp6=data_comp5 %>% ungroup() %>% 
  mutate(Mcov5 = (Mcov4*100/(sum(Mcov4))) ,.by=Structure2) %>% 
     mutate(sd2=sqrt(Mcov5))
...
#barplot empilé
ggplot(data_comp6,aes(x=reorder(spp,-Mcov5, function(x){sum(x)}),y=Mcov5,fill=Structure2))+
  geom_bar(stat="identity",position="stack")+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1,size=12,color="black"))+
    theme(axis.text.y = element_text(size=12,color="black"))+xlab("")
ggsave("Taxon Fe 2022.png",width=10,height=8,dpi=350)
...
ggplot(data_comp31Bi,aes(x = 0,y=Mcov3,fill=forcats::fct_inorder(spp)))+
  ...
  scale_fill_manual(values=c("#ff9900","#990099","#109628"))+
  theme_void()+
  theme(legend.position="none") 
  ...
  coord_polar("y",start=0)+
  geom_label_repel(aes(label =paste(spp,":",format(Mcov3,digits=1),"%")), position=position_stacknudge(x = .9, y=.2,vjust=.5),size=8,force=25)+
  scale_fill_manual(values=c("tan4","#3366cc","#ff9900","#109628","#0099c6"))+
  theme_void()+
  theme(legend.position="none") 
  ...
  coord_polar("y",start=0)+
  geom_label_repel(aes(label =paste(spp,":",format(Mcov3,digits=1),"%")), position=position_stacknudge(x = .9, y=.2,vjust=.5),size=8,force=25)+
  scale_fill_manual(values=c("#dd4477","#66aa00","#109628","#0099c6"))+
  theme_void()+
  theme(legend.position="none") 
  ...
  coord_polar("y",start=0)+
  geom_label_repel(aes(label =paste(spp,":",format(Mcov3,digits=1),"%")), position=position_stacknudge(x = .9, y=.2,vjust=.5),size=8,force=25)+
  scale_fill_manual(values=c("tan4","#dc3912","#ff9900","#dd4477","#109628"))+
  theme_void()+
  theme(legend.position="none") 
...
data_comp2=dataR %>% ungroup %>% 
  ...
  group_by(spp,Site,Micro,Date_courte,Structure2,saison,Years,Day_delta) %>% 
  # filter(Date_courte %in% c("2021-04","2021-07","2021-11","2022-01","2022-04","2022-11")) %>% #selection des dates ou il y a 4 dates (ou 3 sans fetlar)
  filter(!Date_courte == c("2021-01")) %>%
  # filter(!Site == "Fe") %>%
  distinct() %>% drop_na() %>% 
  summarize(Mcov2 = mean(Mcov,.by=c(spp,Micro,Day_delta,Quad2))) 
  
data_comp3=data_comp2 %>% ungroup %>% 
  select(spp,Site,Structure2,Date_courte,Mcov2,Day_delta) %>% 
    group_by(spp,Site,Structure2,Date_courte,Day_delta) %>% 
  summarize(Mcov3=mean(Mcov2,.by=c(spp,Site,Structure2,Date_courte,Day_delta)))

data_comp31 <-(data_comp3[which(data_comp3$Mcov3>20.0),])
data_comp31=data_comp31 %>% mutate(sitedate=paste(Site,Date_courte,sep = "_"))
data_comp31=data_comp31 %>% mutate(structdate=paste(Structure2,Date_courte,sep = "_"))

data_comp32 <- data_comp31 %>% #recouvrement par site
  group_by(sitedate) %>%
  mutate(Mcov4 = (Mcov3 / sum(Mcov3)) * 100)

data_comp32 <- data_comp31 %>% # recouvrement par structure
  group_by(structdate) %>%
  mutate(Mcov4 = (Mcov3 / sum(Mcov3)) * 100)

data_comp32$sitedate<-fct_relevel(data_comp32$sitedate,c(
  "Bi_2021-04","Bu_2021-04","Fe_2021-04","VB_2021-04",
  "Bi_2021-07", "Bu_2021-07", "Fe_2021-07", "VB_2021-07",
   "Fe_2021-10", "VB_2021-10",
  "Bi_2021-11", "Bu_2021-11", "Fe_2021-11", "VB_2021-11",
  "Bi_2022-01","Bu_2022-01","Fe_2022-01","VB_2022-01",
  "Bi_2022-04","Bu_2022-04","Fe_2022-04","VB_2022-04",
  "Bi_2022-07","Bu_2022-07","Fe_2022-07","VB_2022-07",
  "Bi_2022-11", "Bu_2022-11", "Fe_2022-11", "VB_2022-11"))


palette <- brewer.pal(32, "Set1")
if(length(palette) < 28) {
  palette <- colorRampPalette(brewer.pal(12, "Set1"))(32)
}

ggplot(data_comp32,aes(x=as.factor(Date_courte),y=Mcov4,fill=spp))+
         geom_bar(stat="identity",alpha=.7,aes(fill=spp))+
                    scale_fill_manual(values = palette)+
  theme_bw()+
    theme(axis.text.x = element_text(size=16,colour="black",angle=-70,vjust=0.5,hjust=0.5))+
    theme(axis.text.y = element_text(size=16,colour="black"))+
    theme(strip.text.x = element_text(size = 12,colour="black"))+
   theme(axis.title.y = element_text(size = 14,colour="black"))+
  theme(strip.text.x = element_text(size = 16,colour="black"))+ xlab("Dates")+ylab("Pourcentage de recouvrement")+
  facet_wrap(~Structure2)
ggsave("Recouvrement taxons sup20pour par site_date.png",width=15,height=8,dpi=350)
ggsave("Recouvrement taxons sup20pour par structure_date.png",width=15,height=8,dpi=350)
...
datamean<-datatern2 %>%  group_by(Compasite,Date1) %>% 
  dplyr::summarize(Similarité=mean(Similarité),
            Remplacement=mean(Remplacement),
            Richesse=mean(Richesse))

datamean$Compasite<-fct_relevel(datamean$Compasite,c("Bu/Bi","VB/Bu","VB/Bi","Fe/Bi","Fe/Bu","VB/Fe"))

ggplot(datamean,aes(Date1,Similarité,color=Compasite))+
  geom_point()+
  scale_color_manual(values = c("red","orange","gold1", "darkblue", "lightblue","darkgreen"))+
  theme_bw()



datatern2$Compasite<-fct_relevel(datatern2$Compasite,c("Bu/Bi","VB/Bu","VB/Bi","Fe/Bi","Fe/Bu","VB/Fe"))

ggplot(datatern2,aes(Date1,Similarité,fill=Compasite))+
  geom_boxplot(outlier.shape=NA)+
    stat_summary(fun = mean, geom = "point", size = 2, color = "black",position = position_dodge(width = .75))+
  scale_fill_manual(values = c("red","orange","gold1", "darkblue", "lightblue","darkgreen"))+
  theme_bw()
ggsave("Similarité selon compasite et date.png",width=15,height=8,dpi=350)
