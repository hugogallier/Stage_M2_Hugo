...
datamean<-datatern2 %>%  group_by(Compasite,Compastruct,Comp_date,Comp_day) %>%  #permet de faire la moyenne de chaque colonne par site/struct
  #  Compastruct,,Comp_site,Compadate,Compadate,Compadate,Compastruct,Compastruct,Site_Years <-à utiliser dans la ligne d'avant afin de faire les comparaisons voulues
  dplyr::summarize(Similarité=mean(Similarité),
            Remplacement=mean(Remplacement),
            Richesse=mean(Richesse))
...
databetaindex$Comp_day<-as.factor(databetaindex$Comp_day)

model1<-gam(BIvalue~s(Comp_date,bs='re')+s(Compastruct,bs="re")+s(Compasite,bs="re")+s(Betaindex,bs="re")+Compasite*Betaindex,data=databetaindex) 

model1<-gam(BIvalue~s(Comp_day)+s(Compastruct,bs="re")+s(Compasite,bs="re")+s(Betaindex,bs="re")+Compasite*Betaindex,data=databetaindex) 
summary(model1)

plot_smooths(model=model1, series=Comp_day,comparison=Betaindex)+theme_bw() #permet de mettre les gam selon les sites
...
databib$Compasite[databib$Compasite == "Bi/Bi" | databib$Compasite == "Bu/Bu"| databib$Compasite == "Fe/Fe"| databib$Compasite == "VB/VB"] <- "1"
