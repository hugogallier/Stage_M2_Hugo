...
Biz<-c("Bi","Disc","C1B1","0","hiver","0.001")
Buh<-c("Bu","Disc","C1B1","0","hiver","0.001")
Fet<-c("Fe","Disc","C1B1","0","hiver","0.001")
VBa<-c("VB","Disc","C1B1","0","hiver","0.001")
Sitel<-as.data.frame(rbind(Biz,Buh,Fet,VBa))
colnames(Sitel)<-c("Site","Structure2","Micro","Day_delta","saison","S")

Sitel$Site<-as.factor(Sitel$Site)
Sitel$Structure2<-as.factor(Sitel$Structure2)
Sitel$Micro<-as.factor(Sitel$Micro)
Sitel$Day_delta<-as.numeric(Sitel$Day_delta)
Sitel$saison<-as.factor(Sitel$saison)
Sitel$S<-as.numeric(Sitel$S)


dataR2<-rbind(Sitel,dataR2)
rownames(dataR2)<-seq(1:696)
...
datamean<-datatern2 %>%  group_by(Compasite,Compadate) %>%  #permet de faire la moyenne de chaque colonne par site/struct
...
ggtern(data=datatern2, aes(x=Richesse,y=Similarité,z=Remplacement,group=Compadate)) +
  ...
  geom_point(aes(color=Compadate),position=position_jitter_tern(x=0.1, y=0.1, z=0.1),alpha=.1)+
        geom_point(data=datamean,aes(color=Compadate)) +
...
#### Comparaison indices de beta diversité
datatern2=datatern %>%
  # filter(Structure1 == 'Disc') %>%
    # filter(Structure1 %in% c('Disc','Inner_slots')) %>%
    # filter(Structure2 %in% c('Disc','Inner_slots')) %>%
...
datamean<-datatern2 %>%  group_by(Compasite,Compadate,Saison1) %>%  #permet de faire la moyenne de chaque colonne par site/struct
  #  Compastruct,,Comp_site,Compadate,Compadate,Compadate,Compastruct,Compastruct,Site_Years <-à utiliser dans la ligne d'avant afin de faire les comparaisons voulues
  dplyr::summarize(Similarité=mean(Similarité),
            Remplacement=mean(Remplacement),
            Richesse=mean(Richesse))
...
datameansimi=datamean %>% select(Compasite,Compadate,Similarité) 

datameanremp=datamean %>% select(Compasite,Compadate,Remplacement) %>% 

datameanrich=datamean %>% select(Compasite,Compadate,Richesse) %>% 
...
data_comp2=dataR %>% ungroup %>% 
  ...
  ungroup %>% 
  group_by(Site)
  summarize(Mcov3 = mean(Mcov2,.by=c(Site,spp)))
