...
ggplot(PCOA_df, aes(x = x, y = y)) +
  ...
  theme_bw() + geom_label(data=spp.scrs3,label=Abb, nudge_x = 0.25, nudge_y = 0.25, 
    check_overlap = T)
...
datamean<-datatern2 %>%  group_by(Comp_site,Compasitedate,Comp_struct) %>%  #permet de faire la moyenne de chaque colonne par site/struct
...
ggtern(data=datatern2, aes(x=Richesse,y=Similarité,z=Remplacement,group=Compasitedate)) +
  ...
  geom_point(aes(color=Compasitedate),position=position_jitter_tern(x=0.1, y=0.1, z=0.1),alpha=.1)+
        geom_point(data=datamean,aes(color=Compasitedate)) +
