...
```{r}
# Quelle est la vitesse de colonisation de chaque site et comment évoluenr-t-elles ?
...
plot(dataR31$SparT~dataRjour1,col="red",xlim=c(0,800),ylim=c(-0.1,0.1),pch=19,xlab="Nombre de jours",ylab="Nombre moyen d'espèces  colonisant par jour")
par(new=T)
plot(dataR32$SparT~dataRjour2,col="blue",xlim=c(0,800),ylim=c(-0.1,0.1),pch=19,xlab="",ylab="")
par(new=T)
plot(dataR33$SparT~dataRjour3,col="green",xlim=c(0,800),ylim=c(-0.1,0.1),pch=19,xlab="",ylab="")
par(new=T)
plot(dataR34$SparT~dataRjour4,col="black",xlim=c(0,800),ylim=c(-0.1,0.1),pch=19,xlab="",ylab="")
lines(dataR31$SparT~dataRjour1,col="red",xlim=c(0,800),ylim=c(-0.1,0.1),pch=19,xlab="",ylab="")
lines(dataR32$SparT~dataRjour2,col="blue",xlim=c(0,800),ylim=c(-0.1,0.1),pch=19,xlab="",ylab="")
lines(dataR33$SparT~dataRjour3,col="green",xlim=c(0,800),ylim=c(-0.1,0.1),pch=19,xlab="",ylab="")
lines(dataR34$SparT~dataRjour4,col="black",xlim=c(0,800),ylim=c(-0.1,0.1),pch=19,xlab="",ylab="")
...
dataR2=dataR %>% ungroup %>% 
  ...
  distinct()%>% 
  group_by(Site,Structure2,Micro,Date_courte) %>% 
  summarize(S=sum(n(Structure2)),.by=Site) %>%
  na.omit()


seqdate<-c('2021-04','2021-07','2021-11','2022-01','2022-04','2022-07','2022-11')
Bid<-rep(8,times=7)
Bic<-c(8,7,8,8,8,7,8)
Bip<-rep(3,times=7)
Bis<-c(8,8,8,7,8,8,8)
Birep<-rep("Bi",times=7)
Bidata<-cbind(Birep,seqdate,Bid,Bic,Bip,Bis)
...
dataR5<-spread((dataR3=dataR2 %>% 
  filter(Site == 'Bi') %>% 
  group_by(Site,Date_courte) %>% 
  summarize(S2 = mean(S))),key=Site,value=S2)
dataR5[is.na(dataR5)]<-0
...
dissimilarité_jaccard <- vegdist(dataR2[,3], method = "jaccard")

dissimilarité_bray <- vegdist(dataR2[,3], method = "bray")
...
# Recouverement par la roche nue
```{r}
...
datatern<-as.data.frame(c(Simi=0.5,NES=nes$statistic, #nestedness
Turn=mean(turn$total)))#turnover
datatern<-cbind(Simi=0.5,NES=nes$statistic,Turn=mean(turn$total))
datatern<-as.data.frame(datatern)

library(ggtern)
ggtern(data=datatern, aes(x=NES,y=Simi,z=Turn)) +
  geom_point() +
  geom_crosshair_tern()+
  labs(title="") +
  theme_rgbw()
