...
model111111<-gam(S2~s(Day_delta)+s(Structure2,bs="re")+s(Site,bs="re")+s(saison,bs="re")+s(Structure2,by=Site,bs='re'),data=dataR3)
...
model111111<-gam(S2~s(Day_delta)+s(Site,bs="re")+s(saison,bs="re"),data=dataR3)
summary(model111111)
...
lmmodel<-gam(Mcov3~s(Day_delta,k=27)+s(Structure2,bs='re')+s(Site,bs="re")+s(saison,bs="re"),data=dataR4)
...
qq_plot=qq_plot(mod, method = "simulate")+
  labs(subtitle = NULL, title=NULL)+
  theme_bw()

df_mod<- data.frame(log_fitted = fitted(mod),
                 residuals  = resid(mod, type = "deviance"))


dev_mod <- ggplot(mod, aes(x = log_fitted, y = residuals))+
    geom_point() +
...
Parametre1_df <- data_slice(mod,
                    param=evenly(param))


# On génére des predictions avec le nouveau df

fv_ds_distancesea <- fitted_values(mod, data = Parametre1_df) 
...
# -geom_rug pour ajouter des ticks pour identifir le nombre de valeur sur un gradient quantitatif
...
# PCOA_df=PCOA_df %>% filter(!Date == "2021-04")
...
colnames(datatern2)[14]<-"Compadate"
...
datamean<-datatern2 %>%  group_by(Comp_site,Compastruct,Compadate) %>%  #permet de faire la moyenne de chaque colonne par site/struct
...
ggtern(data=datatern2, aes(x=Richesse,y=Similarité,z=Remplacement)) +
  # geom_point(aes(colour=Compasite), position=position_jitter_tern(x=0.1, y=0.1, z=0.1),alpha=.3) +
    # geom_point(aes(colour=Compastruct), position=position_jitter_tern(x=0.1, y=0.1, z=0.1),alpha=.3) +
      geom_point(aes(colour=Compadate), position=position_jitter_tern(x=0.1, y=0.1, z=0.1),alpha=.3) +
