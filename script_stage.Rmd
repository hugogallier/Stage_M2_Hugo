...
dataR=dataR1 %>% filter(!spp %in% c("Ni","Unassigned","ponte","Ponte")) %>%
  filter(Site == ("Fe"))%>%
  select (spp,abb,Site,Micro,Date_courte,Mcov,Structure2)

dataR=data_PQ_light %>% filter(!spp %in% c("Ni","Unassigned","ponte","Ponte")) %>%
  filter(Site == ("Fe"))%>%
  filter(Micro == ("C2P5"),
Date_courte =="2022-07") %>%   select (spp,abb,Site,Micro,Date_courte,Mcov,Structure2)
...
test=dataR[is.na(dataR$Date_courte),]
...
dataR<-na.omit(dataR)
seq1<-rep(0,times=5701) #permet de faire la richesse gamma sur tous les sites
...
dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned")) %>% #enlève les NI et unassigned
  select(spp,Site) %>% #sélection des variables à utiliser
  distinct() %>% 
  group_by(Site) %>%  #groupe les valeurs selon la variable (ici Site)
...
dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned")) %>% 
  ...
  distinct() %>% 
  group_by(Quad2,Site,Date_courte) %>% 
  summarize(S=n()) 
...
ggplot(dataR2 %>%  filter(!Structure2 %in% c("Holes","Dome","Upper_slots")) %>% 
  filter(!Date_courte=="2021-01"), 
...
# Bi
resultat4Bi<- na.omit(spread((dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned","ponte","Ponte")) %>%
...
# Bu
resultat4Bu<- na.omit(spread((dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned","ponte","Ponte")) %>%
  filter(!Structure2 %in% c("Dome","Upper_slots","Holes")) %>%
  filter(!Date_courte == ("2021-01"))%>%
  filter(Site == ("Bu"))%>%
  filter(Micro %in% c("C1B1","C1B2","C1B3","C1B4","C1B5","C1B6","C1B7","C1B8"))%>%
...
# Fe
resultat4Fe<- na.omit(spread((dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned","ponte","Ponte")) %>%
  filter(!Structure2 %in% c("Dome","Upper_slots","Holes")) %>%
  filter(!Date_courte == ("2021-01"))%>%
  filter(Site == ("Fe"))%>%
  filter(Micro %in% c("C1B1","C1B2","C1B3","C1B4","C1B5","C1B6","C1B7","C1B8"))%>%
...
# VB
resultat4VB<- na.omit(spread((dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned","ponte","Ponte")) %>%
  filter(!Structure2 %in% c("Dome","Upper_slots","Holes")) %>%
  filter(!Date_courte == ("2021-01"))%>%
  filter(Site == ("VB"))%>%
  filter(Micro %in% c("C1B1","C1B2","C1B3","C1B4","C1B5","C1B6","C1B7","C1B8"))%>%
...
par(mfrow=c(2,2))
#Bi
resultat3Bi<- na.omit(spread((dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned","ponte","Ponte")) %>%
  filter(!Structure2 %in% c("Dome","Upper_slots","Holes")) %>%
  filter(!Date_courte == ("2021-01"))%>%
  filter(Site == ("Bi"))%>%
...
#Bu
resultat3Bu<- na.omit(spread((dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned","ponte","Ponte")) %>%
  filter(!Structure2 %in% c("Dome","Upper_slots","Holes")) %>%
  filter(!Date_courte == ("2021-01"))%>%
  filter(Site == ("Bu"))%>%
  filter(Micro %in% c("C2P1","C2P2","C2P3","C2P4","C2P5","C2P6","C2P7","C2P8"))%>%
...
#Fe
resultat3Fe<- na.omit(spread((dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned","ponte","Ponte")) %>%
  filter(!Structure2 %in% c("Dome","Upper_slots","Holes")) %>%
  filter(!Date_courte == ("2021-01"))%>%
  filter(Site == ("Fe"))%>%
  filter(Micro %in% c("C2P1","C2P2","C2P3","C2P4","C2P5","C2P6","C2P7","C2P8"))%>%
...
#VB
resultat3VB<- na.omit(spread((dataR2=dataR %>% filter(!spp %in% c("NI","Unassigned","ponte","Ponte")) %>%
  filter(!Structure2 %in% c("Dome","Upper_slots","Holes")) %>%
  filter(!Date_courte == ("2021-01"))%>%
  filter(Site == ("VB"))%>%
  filter(Micro %in% c("C2P1","C2P2","C2P3","C2P4","C2P5","C2P6","C2P7","C2P8"))%>%
...
par(new=T)
plot(spVB,col="darkgreen",ci.type="poly",ci.lty=0,ci.col="lightgreen",ylim=c(0,30),xlim=c(0,12),ylab="",xlab="")
par(new=T)
plot(spFe,col="orange",ci.type="poly",ci.lty=0,ci.col="lightyellow",ylim=c(0,30),xlim=c(0,12),ylab="",xlab="")
par(new=T)
plot(spBu,col="red",ci.type="poly",ci.lty=0,ci.col="#FCBBA1",ylim=c(0,30),xlim=c(0,12),ylab="",xlab="")
legend(x = "bottomright", legend = c("Bi","VB","Fe","Bu"),lty = 1, col = c("blue","darkgreen","orange","red"),lwd = 2)

# pool<-specpool(Comp_mat2)
# poolaccum(Comp_mat,permutation=100)
