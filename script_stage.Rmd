...
# Meta data
Meta_data=data_comp %>% select(Site,Micro,Date_courte) #créer jeu de données avec les méta données
...
# Matrice de compo
Comp_mat=data_comp %>% select(-c(abb,Site,Micro,Date_courte)) %>% as.matrix() #enlève les méta données du jeu qui va être utilisée pour la matrice
...
distdataR<-vegdist(Comp_mat2) #library vegan - distance de Bray-Curtis, considéré face aux distances euclidiennes comme                                   plus efficace pour les données écologiques
