...
sDlengthpre = sDlength %>% select("premier","Structure","Site") %>% mutate(Numinv=1)
colnames(sDlengthpre)[1]<-"value"

sDlengthdeu = sDlength %>% select("deuxieme","Structure","Site") %>% mutate(Numinv=2)
colnames(sDlengthdeu)[1]<-"value"

sDlengthtro = sDlength %>% select("troisieme","Structure","Site") %>% mutate(Numinv=3)
colnames(sDlengthtro)[1]<-"value"

sDlengthqua = sDlength %>% select("quatrieme","Structure","Site") %>% mutate(Numinv=4)
colnames(sDlengthqua)[1]<-"value"

sDlengthcin = sDlength %>% select("cinquieme","Structure","Site") %>% mutate(Numinv=5)
...
ggplot(sDlengthtot, aes(Numinv,value,colour=Structure))+ 
  ...
  geom_line()+
  theme_bw()+ 
  xlab("numéro d'inventaire")+ylab("Angles des vecteurs entre deux dates")+ # pas sûr des dates parce que possible que les structures n'ont pas été échantillonnées tout le temps
facet_wrap(~Site)
...
sDAnglepre = sDAngle %>% select("premier","Structure","Site") %>% mutate(Numinv=1)
colnames(sDAnglepre)[1]<-"value"

sDAngledeu = sDAngle %>% select("deuxieme","Structure","Site") %>% mutate(Numinv=2)
colnames(sDAngledeu)[1]<-"value"

sDAngletro = sDAngle %>% select("troisieme","Structure","Site") %>% mutate(Numinv=3)
colnames(sDAngletro)[1]<-"value"

sDAnglequa = sDAngle %>% select("quatrieme","Structure","Site") %>% mutate(Numinv=4)
