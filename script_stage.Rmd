...
dataR2=dataR %>% ungroup %>% 
  filter(!Date_courte=="2021-01") %>%
  select(spp,Site,Date_courte,Structure2,Day_delta,saison) %>% 
  distinct() %>% 
  group_by(Site,Day_delta,Structure2,Date_courte,saison) %>% #filtre selon le site, date et structure
  summarize(S=n()) 
 
glm1<-glm(S~as.numeric(Day_delta)+ Site,data=dataR2,family=poisson)
plot(glm1)
summary(glm1)
```
...
dataR2=dataR %>% ungroup %>% 
  ...
  select(spp,Site,Date_courte,Structure2,Day_delta,saison) %>% 
  distinct() %>% 
  group_by(spp,Site,Day_delta,Structure2,Date_courte,saison) %>% #filtre selon le site, date et structure
  summarize(S=n()) 
...
model1<-gam(S~s(Day_delta)+Site+Structure2+saison,data=dataR2,method='REML')
...
plot(model1,shift=mean(dataR2$S),residuals=T,cex=1,pch=16,col="blue",shade=T,shade.col='grey')
ggplot(data = dataR2, mapping = aes(x = Day_delta, y = S)) +
