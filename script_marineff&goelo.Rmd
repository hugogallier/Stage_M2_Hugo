...
dataR3=dataRGM %>% ungroup %>%
  ...
  # mutate(num=seq(1:nrow(dataRGM))) %>%
  distinct()%>%
  filter(Structure2 %in% c("Disc","Columns","Pillars")) %>%
...
DF_virtuel4<-DF_virtuel3[,-c(ncol(DF_virtuel3),(ncol(DF_virtuel3)-1),(ncol(DF_virtuel3)-2),(ncol(DF_virtuel3)-3),(ncol(DF_virtuel3)-4))]
DF_virtueltest<-DF_virtuel4[c(1:100),]
...
# colnames(mat.D)<-data_comp2$Name 
rownames(mat.D)<-DF_virtuel3$Name [c(1:100)] 
colnames(mat.D)<-DF_virtuel3$Name [c(1:100)] 

mat.repl=as.matrix(databetadiv2$repl)
mat.repl[upper.tri(mat.repl)]<-NA
diag(mat.repl)<-NA
# rownames(mat.repl)<-data_comp2$Name 
# colnames(mat.repl)<-data_comp2$Name 
rownames(mat.repl)<-DF_virtuel3$Name [c(1:100)] 
colnames(mat.repl)<-DF_virtuel3$Name [c(1:100)] 

mat.rich=as.matrix(databetadiv2$rich)
mat.rich[upper.tri(mat.rich)]<-NA
diag(mat.rich)<-NA
# rownames(mat.rich)<-data_comp2$Name 
# colnames(mat.rich)<-data_comp2$Name 
rownames(mat.rich)<-DF_virtuel3$Name [c(1:100)] 
colnames(mat.rich)<-DF_virtuel3$Name [c(1:100)] 
...
test.mat.D2 <- test.mat.D %>%
  ...
  select(-row_id)  # Supprimer la colonne d'identification
colnames(test.mat.D2)<-c("Date1","Site1","Structure1","inv1","Date2","Site2","Structure2","inv2","Dissimilarité","type")
...
test.mat.repl2 <- test.mat.repl %>%
  ...
  separate(Var2, into = paste0("Var2", 1:4), sep = "_", extra = "drop") %>% #utilise les meta données de la colonne var 2
  select(-row_id)  # Supprimer la colonne d'identification
colnames(test.mat.repl2)<-c("Date1","Site1","Structure1","inv1","Date2","Site2","Structure2","inv2","Remplacement","type")
...
test.mat.rich2 <- test.mat.rich %>%
  ...
  separate(Var2, into = paste0("Var2", 1:4), sep = "_", extra = "drop") %>% #utilise les meta données de la colonne var 2
  select(-row_id)  # Supprimer la colonne d'identification
colnames(test.mat.rich2)<-c("Date1","Site1","Structure1","inv1","Date2","Site2","Structure2","inv2","Richesse","type")



#Création jeu de données rassemblant les différentes données
datadissi<-test.mat.D2 %>% select(Date1,Site1,Structure1,Date2,Site2,Structure2,Dissimilarité) 
datarepl<-test.mat.repl2 %>% select(Date1,Site1,Structure1,Date2,Site2,Structure2,Remplacement) 
datarich<-test.mat.rich2 %>% select(Date1,Site1,Structure1,Date2,Site2,Structure2,Richesse)

datatern<-cbind(datadissi[,-c(7)],Similarité=(1-datadissi[,7])*100,Remplacement=100*datarepl[,7],Richesse=100*datarich[,7])
