...
dataR2=dataR %>% ungroup %>% 
  ...
  # filter(Site == 'recif3') %>% 
  distinct()%>% 
  group_by(Site,Structure2,Micro) %>% 
  summarize(S=n()) %>%
  na.omit()
...
ggplot(dataR2, aes(Site,S, fill=Micro))+ # Avec "fill" ou "col" du peut ajout une séparation de tes données par facteur (ici le site)
  xlab("Structure")+
  ylab("Richesse taxonomique")+
  geom_jitter(alpha=.3)+  # On ajoute les points (pas nécessaire mais utile pour savoir sur quoi se base les boxplots)
  geom_boxplot()+
...
dataR2=dataR %>% ungroup %>% 
  select(spp,Site,Structure2,Date_courte,Micro) %>% 
  # filter(Site == 'recif3') %>% 
  distinct()%>% 
  group_by(Site,Structure2,Date_courte,Micro) %>% 
  summarize(S=n()) %>%
  na.omit()
