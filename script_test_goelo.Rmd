...
dataR2=dataR %>% ungroup %>% 
  select(Site,Structure2,Micro,Day_delta) %>%
  distinct()%>%
  filter(Site=='recif3') %>% 
...
# dataR2=dataR %>% filter(Site == "recif3") %>% ungroup %>%
#   select(spp,Structure2,Mcov) %>% group_by(spp,Structure2) %>%
#   summarize(Mcov=0.2*(mean(Mcov,.by=spp))) %>% #Divisé par 5 afin d'avoir une moyenne si tous les
#   summarize(Mcov=mean(Mcov,.by=spp))
# dataR2$Mcov<-dataR2$Mcov*100/sum(dataR2$Mcov) #permet d'avoir sur 100 pourcents
# 

dataR2=dataR %>% filter(Site == "recif3") %>% ungroup %>%
  select(spp,Mcov) %>% group_by(spp) %>%
  summarize(Mcov=(mean(Mcov,.by=spp)))
dataR2$Mcov<-dataR2$Mcov*100/sum(dataR2$Mcov) #permet d'avoir sur 100 pourcents



ggplot(dataR2,aes(x = 0,y=Mcov,fill=forcats::fct_inorder(spp)))+
  geom_bar(aes(y=Mcov),stat="identity",width=1,col="white")+
  scale_alpha(range=c(1,.2))+
  coord_polar("y",start=0)+
  geom_label_repel(aes(label =paste(spp,":",format(Mcov,digits=1),"%")), position=position_stacknudge(x = .9, y=.2,vjust=.5),size=3,force=25)+
  theme_void()+
...
dataR3
test1<-multipatt(data_comp2[-c(1:5)],data_comp2$Site,max.order=1)
summary(test1)
...
data2MN$Site<-as.factor(data2MN$Site)
data2MN$Quadrat<-as.factor(data2MN$Quadrat)
data2MN$Taxon<-as.factor(data2MN$Taxon)
colnames(data2MN)[3]<-"spp"

data_comp3=data2MN %>% ungroup %>% 
...
PCOA_dfMN=data.frame(x=pcoa_tot_SS$points[,1],y=pcoa_tot_SS$points[,2],
                   Site=Meta_data$Site,
                   Quadrat=Meta_data$Quadrat)


ggplot(data=PCOA_dfMN,aes(x,y,colour=Site))+ # <- Ici on explore avec col les variable qui explique le + l'explosion du nuage de points
  # geom_point(aes(col=Site), size=1, alpha=.6)+
  geom_point(alpha=.3)+
...
# # Test comparaison GOELO vs milieu naturel
```{r}
...
data_comp2=data_comp2 %>% ungroup %>% select(spp,Site,Structure2,S) %>% group_by(spp,Site,Structure2) 
...
data12$Quadrat[which(data12$Site == "T2S2" & data12$Quadrat == "9")]<-"19"
data12$Quadrat[which(data12$Site == "T2S2" & data12$Quadrat == "10")]<-"20"
data12$Site[which(data12$Site == "T2S2")]<-"T2"
